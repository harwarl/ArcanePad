...
contract IDOFactory is IFactory, Ownable {
    ...
    function createPool(PoolConfig calldata poolConfig, VestingConfig calldata vestingConfig, bytes32 whitelistRoot) external returns (address){
...
function _validatePoolConfig(PoolConfig calldata poolConfig) internal view {
    require(
        poolConfig.tokenAddress != address(0),
        "IDOFactory: Token address cannot be zero"
    );
    require(
        _isContract(poolConfig.tokenAddress),
        "IDOFactory: Token address must be a contract"
    );
    require(
        poolConfig.paymentToken != address(0),
        "IDOFactory: Payment token address cannot be zero"
    );
    require(
        _isContract(poolConfig.paymentToken),
        "IDOFactory: Payment token must be a contract"
    );
    require(
        poolConfig.tokenAddress != poolConfig.paymentToken,
        "IDOFactory: Token and payment token must be different"
    );
    require(
        poolConfig.tokenPrice > 0,
        "IDOFactory: Token price must be greater than 0"
    );
    require(
        poolConfig.softCap > 0,
        "IDOFactory: Soft cap must be greater than 0"
    );
    require(
        poolConfig.hardCap > poolConfig.softCap,
        "IDOFactory: Hard cap must be greater than soft cap"
    );
    require(
        poolConfig.hardCap >= poolConfig.softCap * 2,
        "IDOFactory: Hard cap should be at least 2x soft cap"
    );
    require(
        poolConfig.minContribution > 0,
        "IDOFactory: Min contribution must be greater than 0"
    );
    require(
    ...
    require(
        ...
        "IDOFactory: Start time must be in the future"
    );
    require(
        poolConfig.startTime <= block.timestamp + 365 days,
        "IDOFactory: Start time too far in future (max 1 year)"
    );
    require(
        poolConfig.endTime > poolConfig.startTime,
        "IDOFactory: End time must be after start time"
    );
    uint256 saleDuration = poolConfig.endTime - poolConfig.startTime;
    require(
        saleDuration >= 1 hours,
        "IDOFactory: Sale duration too short (minimum 1 hour)"
    );
    require(
        saleDuration <= 30 days,
        "IDOFactory: Sale duration too long (maximum 30 days)"
    );
    require(
        poolConfig.vestingType <= 3,
        "IDOFactory: Invalid vesting type (must be 0-3)"
    );
    require(
...
function _validateVestingConfig(VestingConfig calldata vestingConfig) internal pure {
    require(
        vestingConfig.tgePercent <= 10000, 
        "IDOFactory: TGE percent cannot exceed 100% (10000 basis points)"
    );
    if (vestingConfig.tgePercent == 10000) {
        require(
            vestingConfig.cliff == 0 && 
            vestingConfig.vestingDuration == 0 && 
            vestingConfig.vestingInterval == 0,
    ...
    require(
        vestingConfig.vestingDuration > 0,
        "IDOFactory: Vesting duration must be greater than 0 when TGE < 100%"
    );
    require(
        vestingConfig.vestingInterval > 0,
        "IDOFactory: Vesting interval must be greater than 0"
    );
    require(
    ...
    require(
        vestingConfig.vestingInterval >= 1 days,
        "IDOFactory: Vesting interval too short (minimum 1 day)"
    );
    require(
        vestingConfig.vestingInterval <= 365 days,
        "IDOFactory: Vesting interval too long (maximum 365 days)"
    );
    require(
        vestingConfig.vestingDuration >= 7 days,
        "IDOFactory: Vesting duration too short (minimum 7 days)"
    );
    require(
    ...
    require(
        vestingConfig.cliff <= 365 days,
        "IDOFactory: Cliff period too long (maximum 1 year)"
    );
}
